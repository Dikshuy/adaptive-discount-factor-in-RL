...
def Q_learning(env, env_eval, options, Q, gamma, eps, alpha, max_steps, eval_steps, adaptive_gamma, _seed):
    ...
    return Q, exp_ret, steps_per_episode, gamma_trend
            ...
            for j, alpha in enumerate(args.alpha_values):
                for k, init_value in enumerate(args.initial_values):
                    for seed in range(args.n_seeds):
                        ...
                        Q, exp_ret, steps, gamma_trend = Q_learning(env, env_eval, options, Q, gamma, 1.0, alpha, args.max_steps, args.eval_steps, args.adaptive_gamma, seed)
                    ...
                    error_shade_plot(
                        axs1, results_exp_ret[i, j, k], stepsize=1, smoothing_window=10,
                        label=label, linestyle=linestyles[j % len(linestyles)], color=color
                    )
                    ...
                    if args.adaptive_gamma:
                        ...
                        ax2.set_ylabel("Gamma (γ)", fontsize=10)
                        ax2.set_ylim([0, 1])
                        ax2.grid(True, which="both", linestyle="--", linewidth=0.5)
                        ax2.minorticks_on()
                        ax2.plot(gamma_trend, label=label, color=color, linestyle=linestyles[j % len(linestyles)])
                        ax2.legend(fontsize="small", loc="best")
                        fig2.savefig(f"{output_dirs[env_name]}/gamma_trend.png", dpi=300)

        axs[0].legend(fontsize="small", loc="best")
        axs[1].legend(fontsize="small", loc="best")
        fig1.savefig(f"{output_dirs[env_name]}/exp_ret.png", dpi=300)
        fig2.savefig(f"{output_dirs[env_name]}/steps.png", dpi=300)
        plt.show()
        plt.close()
